...
class StutterModelSystem:
    ...
    Includes full metrics suite for Accuracy, Precision, Recall, and F1-score.
    """
    ...
    def calculate_metrics(self, df):
        """Calculates Accuracy, Precision, Recall, and F1 based on tag matches."""
        total_actual = 0
        total_predicted = 0
        ...
        # We consider each utterance a "sample" for sentence-level accuracy
        # Or we can calculate tag-level accuracy (how many predicted tags were correct out of all predicted)
        # In research, Accuracy = (TP + TN) / Total. 
        # For sequence labeling, we often look at exact match accuracy of the whole sentence.
        ...
        for _, row in df.iterrows():
            ...
            if sentence_correct and local_correct == len(actual_tags):
        ...
        # Accuracy calculation: Correct Tags / Total Tags (Actual + False Positives)
        # Or more standardly: Correct / Total Predicted
        # Let's provide Sentence-level Accuracy as it's common in sequence tasks
        sentence_accuracy = exact_sentence_matches / len(df) if len(df) > 0 else 0
        
        # Tag-level accuracy (Strict)
        tag_accuracy = correct_matches / max(total_actual, total_predicted) if max(total_actual, total_predicted) > 0 else 0
        ...
        return {
            "Strict": {"Precision": precision, "Recall": recall, "F1": f1, "TagAccuracy": tag_accuracy},
...
OVERALL ACCURACY:
- Sentence-Level Accuracy: {metrics['SentenceAccuracy']:.4f}
- Tag-Level Accuracy (Strict): {metrics['Strict']['TagAccuracy']:.4f}
